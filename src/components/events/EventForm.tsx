...
import { IEventFormSchema } from '@/validations/events/event-validation';
import { IEvent } from '@/types/events/event.types';
import RichTextInput from '@/components/RichTextEditor';
import { useGetEventCategoriesForSelectQuery } from '@/redux/events/category-api';
import { format, parseISO, isValid } from 'date-fns';
import { parseBoolean } from '@/utils/parse-booleans';

interface IEventFormProps {
  form: UseFormReturn<IEventFormSchema>;
  onSubmit: (data: IEventFormSchema) => Promise<void>;
  isLoading: boolean;
...
export default function EventForm({
  ...
  const { data: categories } = useGetEventCategoriesForSelectQuery();
  ...
  return (
    <div className="w-full">
      <BackLink href="/dashboard/events" className="mb-4">
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <p className="text-sm text-muted-foreground">
                        This will appear in event previews and search results
                      </p>
                    </FormItem>
                  )}
                />
                      ...
                      <FormControl>
                        <div className="rounded-lg overflow-hidden border border-slate-300 dark:border-slate-600">
                          <RichTextInput
                            ...
                            maxLength={10000}
                            required={true}
                            placeholder="Start writing your event content here..."
                          />
                        </div>
                      </FormControl>
                      <FormMessage />
                      ...
                      render={({ field }) => (
                        <FormItem className="flex flex-row items-start space-x-3 space-y-0 bg-white dark:bg-slate-800 p-4 rounded-lg border border-slate-200 dark:border-slate-600">
                          ...
                          <div className="space-y-1 leading-none">
                            ...
                            <p className="text-sm text-muted-foreground">
                              Make this event visible to readers
                            </p>
                          </div>
                        </FormItem>
                      )}
