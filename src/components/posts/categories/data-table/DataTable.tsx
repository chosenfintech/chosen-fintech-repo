...
import { CategoryTableFilters } from './Filters';
import { DataTablePagination } from '@/components/ui/DataTablePagination';
import { ConfirmationDialog } from '@/components/ui/confirmation-dialog';
import { extractApiError } from '@/utils/extract-api-error';
import { ICategoriesDataTableProps } from '@/types/posts/category.types';
...
export function CategoriesDataTable({
  ...
  const columns = React.useMemo(() => createCategoryColumns(), []);
  ...
  const handleCreateCategory = () => {
    router.push('/dashboard/posts/categories/create');
  };
  ...
  const handleDeleteSelectedCategories = async () => {
    ...
    try {
      const deletePromises = selectedRows.map((row) =>
        deleteCategory(row.original.id).unwrap(),
      );
  ...
  if (noDataAtAll) {
    return (
      <EmptyState
        ...
        description="Get started by creating your first category. Organize your posts and content with custom categories."
