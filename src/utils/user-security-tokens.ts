...
/**
 * Issues a new security token for a user, invalidating any outstanding
 * unconsumed tokens of the same type first (one live token per type).
...
/**
 * Verifies a user-scoped OTP (2FA login / 2FA setup). Wrong codes count
 * against `maxAttempts`; a correct code is consumed atomically so it can
 * never be replayed.
 */
...
export const clearTwoFactorTokens = async (userId: string): Promise<void> => {
  await prisma.userSecurityToken.deleteMany({
    where: {
      userId,
      type: {
